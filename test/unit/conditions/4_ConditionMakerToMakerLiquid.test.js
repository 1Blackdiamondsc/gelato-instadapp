...
describe("ConditionMakerToMakerLiquid Unit Test", function () {
  this.timeout(0);
  ...
  let conditionMakerToMakerLiquid;
  ...
  beforeEach(async function () {
    ...
    // ========== Test Setup ============
    conditionMakerToMakerLiquid = await ethers.getContract(
  ...
  it("#1: ok should return Ok if for new vault (to open) the borrow amnount is less than the remaining debt that can be borrowed", async function () {
    ...
    const conditionData = await conditionMakerToMakerLiquid.getConditionData(
      dsa.address,
      cdpAId,
      0,
      "ETH-B"
    ...
    expect(
      await conditionMakerToMakerLiquid.ok(0, conditionData, 0)
    ).to.be.equal("OK");
  });
  ...
  it("#2: ok should return DebtCeilingReached if for new vault (to open) the borrow amnount is greater than the remaining debt that can be borrowed", async function () {
    ...
    expect(
      await conditionMakerToMakerLiquid.ok(0, conditionData, 0)
    ).to.be.equal("MakerDebtCeilingReached");
  });
