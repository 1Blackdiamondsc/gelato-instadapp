...
describe("ConditionMakerToAaveLiquid Unit Test", function () {
  this.timeout(0);
  ...
  it("#1: ok should return Ok if Aave has enough liquidity for the futur borrow", async function () {
    ...
    const conditionData = await conditionMakerToAaveLiquid.getConditionData(
      cdpAId
    );
    expect(
      await conditionMakerToAaveLiquid.ok(0, conditionData, 0)
    ).to.be.equal("OK");
  });

  it("#2: ok should return AaveIlliquid if Aave hasn't enough liquidity for the futur borrow", async function () {
    // Steps :
    // 1 - Deposit.
    // 2 - Borrow.
    // 3 - Test if Aave has enough liquidity for the futur borrow.
