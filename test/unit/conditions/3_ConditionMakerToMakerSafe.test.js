...
describe("ConditionMakerToMakerSafe Unit Test", function () {
  this.timeout(0);
  ...
  //Use Maker instead of Condition for checking the behind logic
  let makerResolver;
  ...
  beforeEach(async function () {
    ...
    // ========== Test Setup ============
    conditionMakerToMakerSafe = await ethers.getContract(
  ...
  it("#1: ok should return Ok when the gas fees didn't exceed a user define amount with vault creation", async function () {
    ...
    const conditionData = await conditionMakerToMakerSafe.getConditionData(
      dsa.address,
      cdpAId,
      0,
      "ETH-B"
    );
    expect(await conditionMakerToMakerSafe.ok(0, conditionData, 0)).to.be.equal(
      "OK"
    );
  });
  ...
  //   );
  //   expect(
  //     await conditionMakerToMakerSafe.ok(0, conditionData, 0)
  ...
  it("#8: New Vault Case : destVaultWillBeSafeExplicit should return false when col is lower than borrow amount / spot", async function () {
    ...
    expect(
      await makerResolver.newVaultWillBeSafe(
        "ETH-B",
        amountOfColToDepo,
        amountToBorrow
      )
  ...
  it("#10: Old Vault Case : destVaultWillBeSafeExplicit should return false when col is lower than borrow amount / spot", async function () {
    ...
    expect(
      await makerResolver.vaultWillBeSafe(
        cdpBId,
        amountOfColToDepo,
        amountToBorrow
      )
