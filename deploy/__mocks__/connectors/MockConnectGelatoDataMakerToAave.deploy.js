...
module.exports = async (hre) => {
  if (hre.network.name === "mainnet") {
    console.log(
      "\n\n Deploying MockConnectGelatoDataMakerToAave to mainnet. Hit ctrl + c to abort"
    );
  ...
  if (hre.network.name === "hardhat") {
    ...
    await deploy("MockConnectGelatoDataMakerToAave", {
      from: deployer,
      args: [
        connectorId,
    ...
    await instaConnectors
      .connect(instaMaster)
      .enable(
        (await ethers.getContract("MockConnectGelatoDataMakerToAave")).address
      );
    ...
    // the following will only deploy "MockConnectGelatoDataMakerToAave"
