...
describe("Security: _cast function by example of ETHA-ETHB with disabled ConnectBasic", function () {
  this.timeout(0);
  ...
  before(async function () {
    ...
    const result = await setupETHAToCompound();
  ...
  it("#3: Test Case: Task execution with LogExecReverted due to disabled connector", async function () {
    ...
    const connectBasic = await hre.network.config.ConnectBasic;

    expect(await instaConnectors.isConnector([connectBasic])).to.be.true;
    ...
    await instaConnectors.connect(instaMaster).disable(connectBasic);
    ...
    expect(
      logs.some(
        (log) =>
          ...
          "GelatoCore._exec:ConnectGelatoDataMakerToCompound-v1.0._cast:not-connector"
      )
    ).to.be.true;
  });
});
