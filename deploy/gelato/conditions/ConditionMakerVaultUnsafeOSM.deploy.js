...
module.exports = async (hre) => {
  if (hre.network.name === "mainnet") {
    console.log(
      "\n\n Deploying ConditionMakerVaultUnsafeOSM to mainnet. Hit ctrl + c to abort"
    );
    await sleep(10000);
  }
  ...
  if (hre.network.name === "hardhat") {
    ...
    // the following will only deploy "ConditionMakerVaultUnsafeOSM"
    // if the contract was never deployed or if the code changed since last deployment.
    await deploy("ConditionMakerVaultUnsafeOSM", {
      from: deployer,
    });
    ...
    await osm.kiss(
      (await ethers.getContract("ConditionMakerVaultUnsafeOSM")).address
    );
...
module.exports.tags = ["ConditionMakerVaultUnsafeOSM"];
