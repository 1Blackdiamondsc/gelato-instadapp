...
const setupFromETHA = require("../helpers/setupFromETHA");
const getInstaPoolV2Route = require("../../../../helpers/services/InstaDapp/getInstaPoolV2Route");
const getGasCost = require("../helpers/constants/getGasCostETHAToAave"); // We suppose we will use the aave debt bridge route
...
describe("Full Debt Bridge refinancing ETHA => Aave by multi protocol", function () {
  this.timeout(0);
  ...
  before(async function () {
    ...
    const result = await setupFromETHA();
  ...
  it("#2: User submits Debt refinancing task if market move to Gelato via DSA", async function () {
    ...
    const conditionMakerVaultUnsafeObj = new GelatoCoreLib.Condition({
      inst: contracts.conditionMakerVaultUnsafeOSM.address,
      data: await contracts.conditionMakerVaultUnsafeOSM.getConditionData(
        vaultId,
    ...
    const conditionCanDoRefinanceObj = new GelatoCoreLib.Condition({
      inst: contracts.conditionCanDoRefinance.address,
      data: await contracts.conditionCanDoRefinance.getConditionData(
    ...
    const refinanceIfVaultUnsafe = new GelatoCoreLib.Task({
      conditions: [conditionMakerVaultUnsafeObj, conditionCanDoRefinanceObj],
    ...
    await contracts.dsa.cast(
      ...
      [
        await hre.run("abi-encode-withselector", {
          ...
          inputs: [vaultId, ethers.utils.parseEther("6"), 0, 0],
        }),
      ],
      wallets.userAddress
    );
